...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # 👇 CHANGED: t0 is now a dropdown with exactly the 4 values needed
      selectInput(
        "t0",
        "Time t₀ for distribution of S(t₀):",
        choices = c("10" = 10, "100" = 100, "1000" = 1000, "10000" = 10000),
        selected = 10
      ),
...
server <- function(input, output, session) {
  ...
  output$histPlot <- renderPlot({
    ...
    hist(S_t0, breaks = 50, freq = FALSE,
         main = paste0("Histogram of S(t₀) at t₀ = ", t0),
         xlab = "S(t₀)")
    
    m_theory <- lambda * t0 / beta
